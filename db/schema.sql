...
-- The hash-chain head. Read on EVERY ledger write to find prev_hash, so
-- without this it is a sequential scan whose cost grows with history and
-- writes get slower the longer the system is used. Measured before adding:
-- 29.9 ms at 500k events (infra/loadtest.py). Also serves sequence-range
-- reads for audit export.
CREATE INDEX flow_event_sequence_idx
